...
def generate_index():
    ...
    #print("Sesion Folders:", sesion_folders)  # Debugging
    
    # Sort session folders based on session number
    sesion_folders.sort(key=lambda x: get_session_number(x))
    
    #print("Sorted Sesion Folders:", sesion_folders)  # Debugging
    ...
    #print("Sesion Blocks:", session_blocks)  # Debugging
    ...
    with open(notes_path, 'w', encoding='utf-8') as index_file:
        for folder in sesion_folders:
            ...
            index_file.write(f"## Sesion {session_number}: {folder}\n\n")
